...
const nodemailer = require("nodemailer"); // FIXED: Added missing nodemailer import
...
app.use(express.json()); // FIXED: Replaced deprecated bodyParser
...
// MongoDB Connection
mongoose
  .connect(process.env.MONGO_URI) // FIXED: Removed deprecated options
...
const QuickBookingSchema = new mongoose.Schema(
  {
    ...
    type: { type: String, default: "quick_booking" },
  },
  { timestamps: true }
);
  ...
  try {
    ...
    if (existingUser) {
      if (!existingUser.isVerified) {
        ...
        const mailOptions = {
          from: `"VOAT Network" <${process.env.EMAIL_USER}>`,
          to: existingUser.email,
          subject: "Verify Your Account on VOAT Network",
          html: `<p>Hello ${existingUser.name},</p><p>Your One-Time Password (OTP) is: <h2><b>${otp}</b></h2></p><p>This is valid for 10 minutes.</p>`,
        };
        await transporter.sendMail(mailOptions);
        return res.status(200).json({
          success: true,
          message:
            "Email is registered but not verified. A new OTP has been sent.",
    ...
    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    savedUser.otp = otp;
    savedUser.otpExpires = Date.now() + 10 * 60 * 1000;
    await savedUser.save();

    const mailOptions = {
      from: `"VOAT Network" <${process.env.EMAIL_USER}>`,
      to: savedUser.email,
      subject: "Welcome to VOAT Network! Verify Your Email",
      html: `<p>Hello ${savedUser.name},</p><p>Your One-Time Password (OTP) is: <h2><b>${otp}</b></h2></p><p>This is valid for 10 minutes.</p>`,
    };
    await transporter.sendMail(mailOptions);
...
app.post("/api/send-otp", otpLimiter, async (req, res) => {
  try {
    ...
    const mailOptions = {
      from: `"VOAT Network" <${process.env.EMAIL_USER}>`,
      to: user.email,
      subject: "Your New OTP for VOAT Network",
      html: `<p>Hello ${user.name},</p><p>Your new OTP is: <h2><b>${otp}</b></h2></p><p>This is valid for 10 minutes.</p>`,
    };
    await transporter.sendMail(mailOptions);
    res.status(200).json({
      success: true,
...
app.put("/api/admin/quick-booking/:id/status", async (req, res) => {
  try {
    ...
    if (!quickBooking) {
      // FIXED: Completed the JSON response object
      return res
        .status(404)
        .json({ success: false, message: "Quick booking not found" });
    }
...
app.post("/api/add-service", async (req, res) => {
  try {
    ...
    if (!portfolio) {
      // Create a new portfolio if one doesn't exist for the user
      portfolio = new PortfolioSubmission({
        userId,
        services: [newService],
        status: "pending", // New portfolios should be pending approval
      });
    } else {
      // If portfolio exists, add or update the service
    ...
    // FIXED: Safely find the service and check for its existence before getting the ID.
    const savedService = portfolio.services.find(
      (service) => service.name === name
    );

    // This prevents a server crash if the service isn't found.
    if (!savedService) {
      console.error(
        "Failed to find the service after saving. Portfolio:",
        portfolio
      );
      return res.status(500).json({
        success: false,
        message: "Service was added but could not be retrieved.",
      });
    }
...
app.delete("/api/wishlist/remove/:itemId", async (req, res) => {
  try {
    ...
    // FIXED: Corrected syntax for findOneAndUpdate
    const updatedWishlist = await Wishlist.findOneAndUpdate(
      { userId },
